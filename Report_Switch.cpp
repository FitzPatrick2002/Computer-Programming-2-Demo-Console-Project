...
Report_Switch::Report_Switch(std::string i_p, std::vector<Patient>& pat, std::vector<Report>& repo, std::vector<Report*>& ptr_repo) : Base_Switch(), i_pesel(i_p), all_patients(pat), reports(repo), reports_ptr(ptr_repo) {
...
void Report_Switch::set_switch_data_as_ptr(std::vector<std::string>& arguments, bool why) {
	//Check if this patient even exist
	int patient_index = -1;
	for (int i = 0; i < this->all_patients.size(); i++) {
		if (this->all_patients[i].get_pesel() == this->i_pesel) {
	...
	//std::cout << "Given requierements: " << title << " " << author << " " << date_range << " " << illness << "\n";
	...
	std::vector<Illness>& cured_illnesses = pat.access_cured_illnesses(); // This unfortunately returns a copy :/
	for (auto& ill : cured_illnesses) {
		// Check if we want reports for this kind of illness
		if (illness != "") {
			if (ill.get_name() != illness)
				continue; //Continute to the next illness, that one doesn't match
		...
		for (auto& rep : ill.access_report_history()) {
			...
			//std::cout << "Report title: " << rep.get_title() << "\n";
			//std::cout << "Report author: " << rep.get_author() << "\n";
			...
			if (title_ok && date_ok && author_ok) {
				//this->reports.push_back(rep);
				//std::cout << "Pushing back report\n";
				this->reports_ptr.push_back(&rep);
			}
				
		}
	}

	std::vector<Illness>& on_going_illnesses = pat.acces_on_going_illnesses(); // This unfortunately returns a copy :/
	for (auto& ill : on_going_illnesses) {
		// Check if we want reports for this kind of illness
		if (illness != "") {
			if (ill.get_name() != illness)
				continue; //Continute to the next illness, that one doesn't match
